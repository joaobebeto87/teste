...
const navItems: NavItem[] = [
  ...
  { kind: "divider" },
  {
    kind: "link", href: "/dashboard/sincronizar-djen", label: "Sync Publicações", roles: ["ADMIN"],
    icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>,
  },
  {
    kind: "link", href: "/dashboard/sincronizar-datajud", label: "Sync Andamentos", roles: ["ADMIN"],
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-navy-800 text-white transition-transform duration-200 lg:static lg:z-auto lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="border-b border-white/10 px-5 py-7 flex flex-col items-center">
          <div className="flex items-baseline gap-0 leading-none mb-1">
            <span className="text-white font-display text-2xl font-bold tracking-tight">C</span>
            <span className="text-gold-400 font-display text-2xl font-bold">&amp;</span>
            <span className="text-white font-display text-2xl font-bold tracking-tight">P.A</span>
          </div>
          <p className="text-gold-400 text-[0.55rem] font-medium tracking-[0.35em] uppercase">· Advogados ·</p>
